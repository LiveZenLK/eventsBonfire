...
$num_columns	= 10;
$can_delete	= $this->auth->has_permission('Issue_UMTS_Mobile.Issueset.Delete');
$can_edit		= $this->auth->has_permission('Issue_UMTS_Mobile.Issueset.Edit');
$has_records	= isset($records) && is_array($records) && count($records);

?>
<div class="admin-box">
	<h3>Return UMTS Stick</h3>
   ...
   <br /><br />
	 ...
	 <p><?php echo anchor(SITE_AREA . '/issueset/issue_umts_mobile/' , '<input type="button" value="View Issue Sets" class="btn btn-warning">') ?></p>
	<?php echo form_open($this->uri->uri_string()); ?>
		<table class="table table-striped">
			...
			<tfoot>
				...
				<tr>
					<td colspan="<?php echo $num_columns; ?>">
						...
						<input type="submit" name="delete" id="delete-me" class="btn btn-danger" value="<?php echo lang('bf_action_delete'); ?>" onclick="return confirm('<?php e(js_escape(lang('issue_umts_mobile_delete_confirm'))); ?>')" />
					</td>
				</tr>
				<?php endif; ?>
			</tfoot>
			...
			<tbody>
				...
				<?php if ($can_edit) : ?>
                                        <td><?php echo anchor(SITE_AREA . '/issueset/issue_umts_mobile/edit/' . $record->id, '<span class="icon-pencil"></span>' . ucfirst($customers[$record->parentCustomer]->name)); ?></td>
				<?php else : ?>
					<td><?php echo ucfirst($customers[$record->parentCustomer]->name); ?></td>
				<?php endif; ?>
				...
				<?php } ?>		
                    ...
                    <td><?php echo ucfirst($userslist[$record->parentAdmin]->username)?></td>
                    <td><?php echo anchor(SITE_AREA . '/issueset/issue_umts_mobile/view_detail/'.$record->id , '<input type="button" value="View Detail" class="btn btn-primary">') ?></td>
				</tr>
				<?php
					endforeach;
				else:
